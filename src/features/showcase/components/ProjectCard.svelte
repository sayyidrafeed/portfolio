...
<article class="group relative overflow-hidden rounded-2xl glass-card transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-gravity-500/20">
  <!-- Thumbnail -->
  ...
  <div class="p-6">
    <!-- Featured Badge -->
    {#if project.featured}
      <span class="mb-3 inline-block rounded-full bg-gravity-500/20 px-3 py-1 text-xs font-medium text-gravity-300">
        Featured
      </span>
    {/if}
    
    <h3 class="mb-2 text-xl font-semibold text-white group-hover:text-gravity-300 transition-colors">
      {project.title}
    </h3>
